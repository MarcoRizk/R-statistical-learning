...
## Support Vector Classifier (Linear Boundary)

The e1071 library contains implementations for a number of statistical
learning methods. In particular, the svm() function can be used to fit a svm() support vector classifier when the argument kernel="linear" is used. This
...
the classes are not lineary seperable

Next, we fit the support vector classifier. Note that in order
...
value of ?? for the radial basis kernel (9.24).
...
the training data using the svm() function with a radial kernel and ?? = 1:
...
X = (X1, X2,...,Xp)T takes the form ??^0 + ??^1X1 + ??^2X2 + ... + ??^pXp.
...
```
SVM appears to be producing accurate predictions. By increasing ?? we can
...
we compute the ROC curves on the test data, the model with ?? = 2 appears
